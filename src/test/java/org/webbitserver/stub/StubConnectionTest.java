...
public class StubConnectionTest {
    ...
    @Test
    public void keeps_track_of_sent_binary_messages() {
        StubConnection connection = new StubConnection(null);
        connection.send(new byte[]{0x5, 0x6});
        connection.send(new byte[]{0x7, 0x8, 0x9, 0xA}, 1, 2);
        List<byte[]> sent = connection.sentBinaryMessages();
        assertArrayEquals(new byte[][]{new byte[]{0x5, 0x6}, new byte[]{0x8, 0x9}}, sent.toArray());
    }
